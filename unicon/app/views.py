...
def search_products(req):
    keywords = req.GET.get('search_keywords')

    titleResults = Product.objects.filter(title__icontains = keywords)
    categoryResults =  Product.objects.filter(category__icontains = keywords) 
    brandResults =  Product.objects.filter(brand__icontains = keywords)
    descResults =  Product.objects.filter(description__icontains = keywords)
    
    results = titleResults.union(descResults)
    results = results.union(categoryResults)
    results = results.union(brandResults)

    print("=======================", results[0].title)
    return render(req, 'app/search.html', {'results': results})
